...
class NetworksOperation{
  ...
  static void signIn(BuildContext context,String email,String password,String admin) async {
    ...
    try{
      ...
      if(response!=null&&response.statusCode==200){
       ...
       //    prefs.setString("roleId", jsonDecode(response.body)['roles'][0]['roleId']);
       //    prefs.setString("storeId", jsonDecode(response.body)['roles'][0]['storeId']);
       //    prefs.setString("restaurantId", jsonDecode(response.body)['roles'][0]['restaurantId']);
       //    prefs.setString("userId", jsonDecode(response.body)['roles'][0]['userId']);
       //    var claims = Utils.parseJwt(jsonDecode(response.body)['token']);
       //    prefs.setString('nameid', claims['nameid']);
       //    prefs.setString('storeid', claims['storeid']);
       //
       //  });
       ...
       //  }else if(admin == "rider") {
       //    Navigator.pushAndRemoveUntil(context,
       //        MaterialPageRoute(builder: (context) => DeliveryBoyNavBar()), (
       //            Route<dynamic> route) => false);
       //  }
       //  else if(admin == "chef") {
       //    Navigator.pushAndRemoveUntil(context,
       //        MaterialPageRoute(builder: (context) => KitchenDashboard()), (
       //            Route<dynamic> route) => false);
       //  }
       //  else{
       //    Navigator.pushAndRemoveUntil(context,
       ...
       //            Route<dynamic> route) => false);
       //  }
       // print(jsonDecode(response.body)['roles'][0]);
 ...
 static Future<dynamic> getRoles(BuildContext context)async{
  ...
  try{
   // pd.show();
    var response=await http.get(Utils.baseUrl()+"Categories/GetAll/$storeId/1",);//0 is for time limitation
    var data= jsonDecode(response.body);
    if(response.statusCode==200){
      pd.hide();
...
/// DailySession
 ...
 static Future<dynamic> getOrdersByCustomer(BuildContext context,String token)async{
  ...
  static Future<dynamic> getOrderById(BuildContext context,String token,int OrderId)async{
